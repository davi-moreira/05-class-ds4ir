...
shapefile_df %>% filter(CD_GEOCMU != "2605459") %>% 
  ggplot() +
...
<!---
```{r message=FALSE, warning=FALSE}
idh_pe <- pnud %>% select(ANO, UF, Codmun7, IDHM) %>%  filter(Codmun7 != "2605459" & 
                                                                UF == 26 & 
                                                                ANO == 2010)
shapefile_df %>% filter(CD_GEOCMU != "2605459") %>% 
  left_join(idh_pe, by = c("CD_GEOCMU" = "Codmun7")) %>%
  ggplot() +
  geom_polygon(aes(x = long, y = lat, group = group, fill = IDHM), 
               colour = "black", size = .2) + 
theme_minimal() +  
...
```{r message=FALSE, warning=FALSE}
world %>%  ggplot() +
  geom_polygon(aes(x=long, y = lat, group = group), fill="#2D3E50", color="#FEBF57", size = .15) + 
  theme_minimal() +
  coord_fixed(1)
```
